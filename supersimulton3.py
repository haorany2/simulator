...
class Super_simulton3(Simulton):
    radius=5
    check1=False
    check2=False
    check3=False
    ...
    def __init__(self,x,y,angle,speed):
        Simulton.__init__(self,x,y,Super_simulton3.radius,Super_simulton3.radius)  #####how large are these robots, need to dis
        self._angle = angle
        self._speed = speed
...
#         self.wall_bounce()
    ...
    def display(self,canvas):
        ...
        canvas.create_oval(self._x-Super_simulton3.radius      , self._y-Super_simulton3.radius,
        self._x+Super_simulton3.radius, self._y+Super_simulton3.radius,
        fill='green')
    ...
    def update(self): # the way that robots move
        ...
        mw,mh    = model.world()
        if model.scan_time3!=0:
            self.change_location(0,0)
        if  model.avoid_victum_counter3!=0 and model.scan_time3==0:
            print("yes22::::",model.avoid_victum_counter3)
            self._angle+=0.3*math.pi
            self.change_location(-self._speed*math.cos(self._angle),
                              -self._speed*math.sin(self._angle))
        if Super_simulton3.check1==False and model.avoid_victum_counter3==0: 
            Super_simulton3.check1=self.move_to_destiniation(10,390)
        ...
        if Super_simulton3.check1==True and Super_simulton3.check3==False and model.avoid_victum_counter3==0:
            Super_simulton3.check3=True
...
#         
        ...
        for i in list(model.get_static_simultons()):
            ...
            if (i.get_location()[0]-x)**2+(i.get_location()[1]-y)**2<=900 and\
                ...
                model.avoid_victum3=True
                
                print("come inside2")
        
        if model.avoid_victum3==True:
            model.avoid_victum_counter3=4
            model.avoid_victum3=False
            
        ######################build suspect list in lidar raneg#################
        if Super_simulton3.check3==True and Super_simulton3.check4==False:
            for i in list(model.get_static_simultons()):
                if (i.get_location()[0]-x)**2+(i.get_location()[1]-y)**2<=70756 and [i.get_location()[0]-10,i.get_location()[0]+10,i.get_location()[1]-10,i.get_location()[1]+10] not in [[k[0],k[1],k[2],k[3]]for k in model.suspect_victums]:#2/3 is detected with lidar
                    ...
                    Super_simulton3.check4=True# when check4 is true, scan finished  .need to add a counter for scan time   
                    
                    
        if  Super_simulton3.check4==True and self.scan_flag==False:
            model.scan_time3=50########this number need to be test out
            self.scan_flag=True                
                    
        if Super_simulton3.robot_busy==False and Super_simulton3.check4==True and model.scan_time3==0:#need to go confirm session
            ...
            if lst!=None:
                for i in lst:
                    if i[4]==False:
                        i[4]=True
                        Super_simulton3.current_victum_mission=(i[0],i[1],i[2],i[3])
                        Super_simulton3.robot_busy=True
                        break
            else:#shouldnt come to this
                print(lst)
            print("current_victum_mission2:::",Super_simulton3.current_victum_mission)
        if   Super_simulton3.robot_busy==True:  
                Super_simulton3.finish_mission=(self.move_close_to_destiniation(Super_simulton3.current_victum_mission[0],Super_simulton3.current_victum_mission[2]))
            
        if   Super_simulton3.finish_mission:
            Super_simulton3.robot_busy=False
            Super_simulton3.finish_mission=False
            for i in list(model.static_simultons):
                #if (i.get_location()[0]-x)**2+(i.get_location()[1]-y)**2<=225:##need to change the num
                if (Super_simulton3.current_victum_mission[1]-Super_simulton3.current_victum_mission[0])/2+Super_simulton3.current_victum_mission[0]==i.get_location()[0] and (Super_simulton3.current_victum_mission[3]-Super_simulton3.current_victum_mission[2])/2+Super_simulton3.current_victum_mission[2]==i.get_location()[1]:
