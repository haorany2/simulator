...
class Super_simulton1(Simulton):
    radius=5
    check1=False
    check2=False
    check3=False
    ...
    def __init__(self,x,y,angle,speed):
        Simulton.__init__(self,x,y,Super_simulton1.radius,Super_simulton1.radius)  #####how large are these robots, need to dis
        self._angle = angle
        self._speed = speed
...
#         self.wall_bounce()
    ...
    def display(self,canvas):
        ...
        canvas.create_oval(self._x-Super_simulton1.radius      , self._y-Super_simulton1.radius,
        self._x+Super_simulton1.radius, self._y+Super_simulton1.radius,
        fill='green')
    ...
    def update(self): # the way that robots move
        ...
        if model.scan_time1!=0:
            self.change_location(0,0)
        if  model.avoid_victum_counter1!=0 and model.scan_time1==0:  #the scan_time condition should be useless
            print("yes::::",model.avoid_victum_counter1)
            self._angle+=0.3*math.pi
            self.change_location(-self._speed*math.cos(self._angle),
                              -self._speed*math.sin(self._angle))
        if Super_simulton1.check1==False and model.avoid_victum_counter1==0: 
            Super_simulton1.check1=self.move_to_destiniation(10,390)
        if Super_simulton1.check1==True and Super_simulton1.check2==False and model.avoid_victum_counter1==0:
                Super_simulton1.check2=self.move_to_destiniation(10,10)
        if Super_simulton1.check2==True and Super_simulton1.check3==False and model.avoid_victum_counter1==0:
            Super_simulton1.check3=True
...
#         
        ...
        for i in list(model.get_static_simultons()):
            ...
            if (i.get_location()[0]-x)**2+(i.get_location()[1]-y)**2<=900 and\
                ...
                model.avoid_victum1=True
                
                print("come inside")
        
        if model.avoid_victum1==True:
            model.avoid_victum_counter1=4
            model.avoid_victum1=False
        if Super_simulton1.check3==True and Super_simulton1.check4==False:
            for i in list(model.get_static_simultons()):
                if (i.get_location()[0]-x)**2+(i.get_location()[1]-y)**2<=70756:#2/3 is detected with lidar
                    model.add_suspect_victums([i.get_location()[0]-10,i.get_location()[0]+10,i.get_location()[1]-10,i.get_location()[1]+10,False])
                    Super_simulton1.check4=True# when check4 is true, scan finished  .need to add a counter for scan time   
                    
                    
        if  Super_simulton1.check4==True and self.scan_flag==False:
            model.scan_time1=50########this number need to be test out
            self.scan_flag=True        
        
                    
        if Super_simulton1.robot_busy==False and Super_simulton1.check4==True and model.scan_time1==0:#need to go confirm session
            ...
            if lst!=None:
                for i in lst:
                    if i[4]==False:
                        i[4]=True
                        Super_simulton1.current_victum_mission=(i[0],i[1],i[2],i[3])
                        Super_simulton1.robot_busy=True
                        break
            else:#shouldnt come to this
                print(lst)
            print("current_victum_mission:::",Super_simulton1.current_victum_mission)
        if   Super_simulton1.robot_busy==True:  
                Super_simulton1.finish_mission=(self.move_close_to_destiniation(Super_simulton1.current_victum_mission[0],Super_simulton1.current_victum_mission[2]))
            
        if   Super_simulton1.finish_mission:
            Super_simulton1.robot_busy=False
            Super_simulton1.finish_mission=False
            for i in list(model.static_simultons):
                if (Super_simulton1.current_victum_mission[1]-Super_simulton1.current_victum_mission[0])/2+Super_simulton1.current_victum_mission[0]==i.get_location()[0] and (Super_simulton1.current_victum_mission[3]-Super_simulton1.current_victum_mission[2])/2+Super_simulton1.current_victum_mission[2]==i.get_location()[1]:
